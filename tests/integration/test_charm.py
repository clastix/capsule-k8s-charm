...
async def test_kubernetes_resources_created(ops_test: OpsTest):
    ...
    client.get(Service, name="capsule-controller-manager-metrics-service", namespace=ops_test.model_name)
    client.get(MutatingWebhookConfiguration, name="capsule-mutating-webhook-configuration", namespace=ops_test.model_name)
    client.get(ValidatingWebhookConfiguration, name="capsule-validating-webhook-configuration", namespace=ops_test.model_name)